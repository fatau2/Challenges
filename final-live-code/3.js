...
function findEmptySeat(room) {
  var emptySeatPositions = [];
  for (let i = 0; i < room.length; i++) {
    for (let j = 0; j < room.length; j++) {
      if(room[i][j] !== '*' && j !== Math.round(room.length/2) - 1 && j !== 7){
        var empty = i + "," + j;
        emptySeatPositions.push(empty);
      }
    }
    
  }
  if (emptySeatPositions.length === 0){
    return "maaf tidak ada kursi tersedia"
  } else {
    backMostPostion = emptySeatPositions[emptySeatPositions.length - 1];
    iPositionOfChoice = backMostPostion[0];
    jPositionOfChoice = backMostPostion[2];
    //emptySeatPositions[emptySeatPositions.length - 1];
    //return room;
    //return emptySeatPositions[emptySeatPositions.length - 1];
    room[iPositionOfChoice][jPositionOfChoice] = "#"
    //console.log(iPositionOfChoice + " " + jPositionOfChoice);
