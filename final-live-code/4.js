...
function train(pokemon, candy) {
  var newLevel = pokemon.level + candy;
  //return newLevel;
  if (pokemon.evolutionLine === undefined){
    return "congratulations your " + pokemon.name + " has grown to level " + newLevel;
  } 
  var evolvedFirstStage = "";
  var evolvedSecondStage = "";
    for (let i = 0; i < pokemon.evolutionLine.length - 1; i++) {
      if (pokemon.evolutionLine[i].level <= newLevel){
        evolvedFirstStage = pokemon.evolutionLine[i].name;        
      } 
      if (pokemon.evolutionLine[i + 1].level <= newLevel){
        evolvedSecondStage = pokemon.evolutionLine[i+1].name;  
      }
    }
    if (evolvedSecondStage !== ""){
    return "congratulations your " + pokemon.name + " has grown to level " + newLevel + " and it evolved into " + evolvedFirstStage + " and " + evolvedSecondStage;
    } else {
      return "congratulations your " + pokemon.name + " has grown to level " + newLevel + " and it evolved into " + evolvedFirstStage;
    }
}
