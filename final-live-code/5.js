...
function userWeapon(user, weapon) {
  ...
  for (let i = 0; i < heroes.length; i++) {
    if (heroes[i].job === user.job){
      //console.log(heroes[i].job);
      for (let j = 0; j < heroes[i].weaponTypes.length; j++) {
        //console.log(weapon);
        if (heroes[i].weaponTypes[j] === weapons[weapon].type){
          //console.log(heroes[i].weaponTypes[j]);
          validJob = true;
        }
  ...
  user.weapon = weapon;
  return user.name + " menggunakan senjata " + user.weapon + " attack menjadi " + parseInt(user.attack);
}
